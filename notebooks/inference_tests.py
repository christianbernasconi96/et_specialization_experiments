...
class NewIncrementalDoubleThresholdOrMaxInferenceManager(ThresholdOrMaxInferenceManager):
    ...
    def discretize_output(self, network_output):
        ...
        # apply threshold inference on the predictions of the additional classifier
        discrete_pred_incremental = BaseInferenceManager.discretize_output(self, network_output_incremental)
...
class OldIncrementalDoubleThresholdOrMaxInferenceManager(ThresholdOrMaxInferenceManager):
    def __init__(self, name, threshold_pretraining=0.5, threshold_incremental=0.5, type2id=None, transitive=False):
        super().__init__(name, None, type2id, transitive)
        self.threshold_pretraining = self.init_threshold(threshold_pretraining)
        self.threshold_incremental = self.init_threshold(threshold_incremental)
...
# %%
# prepare classes
threshold_pretraining = .6
threshold_incremental = .7
type2id = {'/person': 0,
          '/location': 1,
          '/organization': 2,
          '/person/actor': 3,
...
old_inference_manager = OldIncrementalDoubleThresholdOrMaxInferenceManager(None,
                            threshold_incremental=threshold_incremental,
                            threshold_pretraining=threshold_pretraining,
                            transitive=False,
                            type2id=type2id,
                            )

new_inference_manager = NewIncrementalDoubleThresholdOrMaxInferenceManager(None,
                            threshold_incremental=threshold_incremental,
                            threshold_pretraining=threshold_pretraining,
                            transitive=False,
                            type2id=type2id,
...
# %%
# prepare fake network output
pretraining_shape = (5, 3)
incremental_shape = (5, 3)

output_pretraining = torch.rand(pretraining_shape)
output_incremental = torch.rand(incremental_shape)

output_for_inference = (output_pretraining, output_incremental)
# %%
# infer
print('NETWORK OUTPUT')
print(torch.hstack(output_for_inference))
print()
old_inferred = old_inference_manager.infer_types(output_for_inference)
print('INFERRED OLD')
print(old_inferred)
print()
new_inferred = new_inference_manager.infer_types(output_for_inference)
print('INFERRED NEW')
print(new_inferred)
print()
print('equals:', (old_inferred == new_inferred).min().item())
